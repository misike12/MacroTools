...
and `src/CsMd/` holds the CS:MD integration (`com.misu.csmd`: 3 actions, 39 variables, 11 events, 3-step config flow; live Counter-Strike 2 state over a loopback GSI listener with Steam discovery, map place names out of the game's own VPKs, plus a one-click cfg installer; positions and place names resolve only from spectator/GOTV feeds because the game withholds them from players, and Simulate injects a Mirage Middle position so the tiles stay verifiable).
