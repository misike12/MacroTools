...
and `src/CsMd/` holds the CS:MD integration (`com.misu.csmd`: 3 actions, 40 variables, 11 events, 4-step config flow; live Counter-Strike 2 state over a loopback GSI listener with Steam discovery, map place names out of the game's own VPKs, plus a one-click cfg installer; positions and place names resolve only from spectator/GOTV feeds because the game withholds them from players, and console position tracking (Console/, polled getpos_exact trigger plus console.log tail, gated on local-alive focus) fills them while playing once the user sets the -condebug launch options; Simulate injects a Mirage Middle position so the tiles stay verifiable).
